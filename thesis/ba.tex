...
widely used \emph{planning domain definition language} (\textsc{PDDL}). For
...
planning is to generate a sequence of actions (a \emph{plan}) that declares
...
a possibility for \emph{extending} \textsc{pddl} was searched and found in Clojure,
...
somehow be modeled in \textsc{pddl}. There are \emph{types} of entities
(hackers, software, pizza), logical states, that means predicates,
...
(that means hack into) software and \emph{eat pizza}.
...
Consider the following problem: \emph{Gary} is a \emph{hungry} hacker who should
...
plan the sequence of required actions (\emph{Who has to eat pizza and
when?}, \emph{When and what to hack?}), leading from the initial state
...
\item[{Problem file}] Description of a concrete problem environment within
...
(\emph{white hats} \emph{black hats}, \emph{gray hats}).

\emph{Software} can be
...
parts:planner and use the generated solution file (\emph{plan}).
...
accordance to the \emph{good principles} of object oriented programming
...
\emph{Gary's Hacker World}, a short, yet adequate task specifications for
the hacker \emph{Gary} that tries to \emph{exploit} vulnerable software.
\section{Domain File}
\label{sec-2-2}
...
\subsection{Define}
\label{sec-2-2-1}
Every domain file starts with \texttt{(define (domain NAME) ...)} where,
\texttt{NAME} is a string that starts with a character, and then contains
further characters(\texttt{a-z}), numbers (\texttt{0-9}), hyphens (\texttt{-}) or
underscores (\texttt{\_}). \textsc{PDDL}'s syntax is case insensitive.
\\
...
In the \texttt{(:types ...)} part, \textsc{pddl} allows for structuring and typing
...
\end{minted}
\caption{\label{gary-types}Always this gary}
\end{listing}

Every \textsc{pddl} domain includes the built-in types \texttt{object} and \texttt{number},
...
general syntax is \texttt{(p ?v1 – t1 ?v2 - t2 ...)}, whereby \texttt{?} followed by
a name, declares a variable, and the expression after the hyphen (\texttt{-})
...
so that \texttt{(p ?v1 ?v2 - t)} is similar to \texttt{(p ?v1 - t ?v2 - t)}.
...
effect part.
\item[{\texttt{:precondition}}] The logical expression that is expressed in this
section has to be \texttt{true}, before an action can be applied.
...
declared here (that means the NAME in \texttt{(:domain NAME)} and \texttt{(define
...
\end{minted}
\caption{The instantiated objects. For example, \texttt{gisela - non-hacker} means that the object gisela is a non hacker.}
\end{listing}
...
of an predicate or action is considered as the \emph{main} argument - so
...
\emph{my\textsc{pddl}} is a extensible, modular system, designed for supporting
knowledge engineers in the process of writing, analyzing and expanding
\textsc{pddl} domains and problems. The following integral parts of \emph{my\textsc{pddl}}
will be presented in the following sections:

\begin{description}
\item[{my\textsc{pddl}-i/f}] A general interface between \textsc{pddl} and Clojure, allowing
...
\item[{Human Planner Interaction}] An interactive \textsc{pddl} environment: speech synthesis and
recognition.
\end{description}
...
file \texttt{p01.pddl} (in folder \texttt{problems}) are filled with basic \textsc{pddl}
...
with \emph{my\textsc{pddl}-gen}, which will save its generated output to these
...
\\ \texttt{README.md} is a Markdown file, which is, amongst others, intended
