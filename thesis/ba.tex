...
\end{minted}
\caption{\label{log-domain}The Logistics domain that contains an error}
\end{listing}
...
critically reviewed in Chapter \ref{ch:related} to set the goals for
\mypddl.
...
Chapter \ref{ch:tools}. Since the main aim in the development of the toolkit
was for it to be easy to use and maintain, it is evaluated with regard
to these criteria in Chapter \ref{ch:evaluation}. The usability was assessed
...
the field concludes this thesis in Chapter \ref{ch:conclusion}.
...
\textsc{pddl}, the following six general design principles \footnote{Based on the in How to Design Classes.} ought to
...
\end{minted}
\caption{The type hierarchy for the \emph{Hacker World}, consisting of different types of persons, furniture, computers, hackers, and software. The elements on the left-hand side (for example \texttt{hacker non-hacker}) are declared subtypes of the right-hand side (\texttt{person}) whereby the type hierarchy is expressed.}
\end{listing}
...
\end{minted}
\caption{This section declares five predicates: the unary predicates \texttt{hungry}, \texttt{vulnerable} and \texttt{exploited}, the binary predicate \texttt{has}, and the 3-ary predicate \texttt{location} that specifies x and y coordinates for a furniture item.}
\end{listing}
\subsection{Actions}
\label{sec-2-2-5}
...
\chapter{Knowledge Engineering Tools for Artificial Intelligence Planning}
\label{sec-4}
\label{ch:tools}
...
The erroneous domain in Chapter \ref{ch:introduction} and the \emph{Hacker World}
and \emph{Gary's Huge Problem}, presented in Chapter \ref{ch:basics}, already
...
models" \cite{shah2013knowledge}. The following sections will present
\mypddl, a highly customizable and extensible modular system, designed
for supporting knowledge engineers in the process of writing,
...
the modeling time can reduced an even partly automated (see \ref{subsec:loc}
...
TODO
\end{minted}
\caption{\label{src:add-construct}A \pddl predicate that was added using the interface}
\end{listing}
...
Figure \ref{fig:syntax-highlighting} displays an extract of the domain
\emph{Coffee}, which was used in the user study for the evaluation of this
tool.

\begin{figure}
\caption{\label{fig:syntax-highlighting}The deliberately erroneous domain \emph{Coffee}  without (a) and with (b) syntax highlighting, using the Sublime Text color scheme \emph{Monokai}. All non-highlighted (white) constructs either contain errors or are specified in the wrong block.}
\hspace*{1.5cm}
...
alternated by changing Sublime Text's color scheme \footnote{The extend to which the syntax highlighting is supported is
...
keyword. Typing \texttt{action} and pressing the tabulator key (\key{\tab}),
...
locations, defined in the \texttt{:init} part. In Clojure, the Euclidean
...
are expressed by arrows from the subtype (here: \emph{laptop}) to the super
type, where the arrow head aims at the super type (here: \emph{computer}).
...
\caption{\label{img:auto-gen}The type diagram that was generated from the \emph{Hacker World} using myPDDL-dia.}
\end{figure}
...
\ref{fig:folder-structure} displays the folder structure after invoking
...
revision, stored in the folder \texttt{domains/} . All folders are created if
necessary.
...
can be seen in Figure xyz \ldots{}:For example, a the \pddl domain file
could declare \texttt{(hungry ?p - person)}, although men and women can be
hungry.
...
\section{Integrated Design Environment (myPDDL-sub)}
\label{sec-4-8}
...
While \emph{snp} and \emph{syn} are devised explicitly for Sublime Text and
...
Control or by copying the files of \mypddl \footnote{The files can be downloaded from \url{https://github.com/Pold87/ba-thesis/}.} into the packages
folder of Sublime Text \footnote{Further information about Sublime Text packages can be found at
\url{http://www.sublimetext.com/docs/3/packages.html}.}. Following, the features can be
...
To evaluate a software means to assess its quality. (TODO: Even though there
exists an international standard (\textsc{iso/iec} 25010) to evaluate
software, most of the eight characteristics that serve as quality
criteria are irrelevant for this project). More appropriate criteria
are supplied by \textcite{shah2013knowledge} who evaluate different
knowledge engineering tools in planning, including, among others,
\itsimple. All in all, they identified seven criteria, the lead
questions of which can be found in Table \ref{tab:criteria}.
...
criterion operationality with functional suitability from the
\textsc{isco/iec} 25010 standard. To assess the functional suitability
and to illustrate where \mypddl fits in with similar tools, it was
considered appropriate to compare it to the other three tools
introduced and discussed in chapter 3, namely \pddlstudio, \itsimple,
and \pddlmode for Emacs. Of the remaining six criteria in Table
\ref{tab:criteria}, collaboration, experience, and debugging were tested
with a user test. The other three criteria, maintenance, efficiency,
...
\mypddl to the three tools already discussed in Chapter \ref{ch:related}:
...
Table \ref{tool-comp} is to illustrate how the four tools compare in terms
...
\emph{Monokai} \footnote{\emph{Monokai} is the default color scheme of Sublime Text and has
...
were given pen and paper and a one page summary (\emph{cheat sheet}) of
...
the order specified in table \ref{tab:order} in the appendix. Two factors
...
participants were given six minutes \footnote{A reasonable time frame tested on two pilots.} to detect as many of the
...
35$\backslash$,\% more found errors with syntax highlighting). The difference
is shown in Diagram \texttt{found-errors-combined}.
...
analysis results in 49~\% more errors being found with the help of
...
Diagram \ref{task-completions-agg} shows the geometric mean of the task
completion time (i.e. excluding wrong answers) for each question with
and without the type diagram generator.

\begin{figure}
\caption{\label{task-completions-agg}Geometric mean of task completion time per Question}
...
\begin{figure}
\caption{\label{task-completions-agg-without-q4}Geometric mean of aggregated task completion time without Question 4}

\end{figure}

Diagram \ref{task-completions-agg-without-q4} illustrates the effect that
...
opposite approach of transforming \textasciidieresis{} diagrams into \pddl files. The
