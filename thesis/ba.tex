...
They are described in detail in Chapter \ref{ch:tools}. Since the main aim
...
\ref{ch:evaluation}. The usability was assessed by means of a user test with
...
\begin{description}
\item[{my\textsc{pddl}-new}] Create a \pddl project
folder structure with \pddl domain and problem skeletons.
\item[{my\textsc{pddl}-gen}] A type diagram generator for
analyzing the structure of \pddl type hierarchies.
\item[{my\textsc{pddl}-loc}] Automated distance calculation for
\pddl locations, specified in a problem file.
\item[{my\textsc{pddl}-syn}] A context-aware syntax highlighting feature.
\item[{my\textsc{pddl}-snp}] Code snippets (templates), which can be
inserted in \pddl files.
\item[{my\textsc{pddl}-sub}] A integrated development environment for the
...
serves as a basis for \emph{new}, \emph{gen} and \emph{loc}. As stated before,
...
process as well as reduce the modeling time (see \ref{subsec:loc} the distance
...
The \emph{read-construct} method thereby makes use of an safe reader that
...
\pddl task specification stands the creation of a new \emph{project},
...
projects \ldots{} and the organization.

\emph{my\textsc{pddl}-new} creates \pddl project folders, on the
basis of a project name (Figure \ref{fig:mypddl-new-project-folder}).


\begin{figure}
\caption{\label{mypddl-new-project-folder-without}The project folder structure created by myPDDL-new. The project-name is chosen by the user and used for the name of the created domain.}
\dirtree\{\%
.1 project-name.
.2 dot.
.2 diagrams.
...
file \texttt{p01.pddl} (in the folder \texttt{problems}) are filled with basic \pddl
...
(see \ref{subsec:gen}Type Diagram Generator (myPDDL-gen)). \\
As one domain file can have multiple problem files, the \texttt{problems}
...
(\cite{shah2013knowledge}), \texttt{README.md} is a Markdown (a plain text
...
start and the end of a larger code block (like \texttt{(:predicates ...)}).
...
Figure XYZ displays an extract of the \emph{Coffee} world, a domain used in
...
By using the inbuilt Sublime Text color scheme \emph{Monokai}, almost all
...
inserted by typing a trigger keyword. Typing \texttt{action} and pressing the
...
binary predicate template \texttt{(pred-name ?x - object ?y - object)}.
...
Every snippet is stored in a separate file, located in the \texttt{PDDL/}
...
\emph{Hacker World} in Chapter 2, defines the predicate \texttt{(location ?f -
...
root function (\texttt{\textbackslash{}sqrt}). An Euclidean distance function that uses the
...
operations (\texttt{+}, \texttt{-}, \texttt{/}, \texttt{*}). These operators can be used in
...
\textsc{pddl }
A possibility would be to declare a requirement \texttt{:math}
...
\caption{After}
\end{listing}
\section{Type Diagram Generator (myPDDL-gen)}
\label{sec-4-7}
\label{subsec:gen}
...
diagram from the \emph{Hacker World} in Chapter 2. In the diagram, types
...
Generalization relationships ("is a", for example "a laptop \emph{is a}
computer") are expressed by arrows from the subtype (here: \emph{laptop})
to the super type, where the arrow head aims at the super type (here:
\emph{computer}). This relationship expresses, that every subtype is also
...
\caption{The type diagram that was generated from the \emph{Hacker World} using myPDDL-gen.}
\end{figure}

In order to create the diagram, \emph{gen} makes use of the PDDl/Clojure
interface that extracts the \texttt{(:types ...)} block. Then, using regular
...
graph (\texttt{digraph}) in the \textsc{dot} language is created and saved in
the folder \texttt{dot/} that is located in the same folder as the
\pddl domain file. The \textsc{dot} file is then passed to dot
and a \textsc{png} diagram is created and saved in the folder
\texttt{diagrams/}. Additionally, the diagram will be immediately opened and
displayed in a window. In addition, a copy of the domain file is
stored in the folder \texttt{domains/}. Every time \emph{my\pddl-gen} is
...
xyz displays the folder structure after invoking \emph{dia} twice on the
\emph{Hacker World}. This way, the type hierarchy and predicate structure
...
\texttt{domains/} folder. All folders are created if necessary.
...
the code snippets into an IDE. While \emph{snp} and \emph{syn} are devised
explicitly for ST and therefore integrated from the outset, the other
tools (\emph{new}, \emph{gen}, \emph{loc}) can be used independently of ST utilizing
the command-line interface and any \pddl file. To provide an
IDE for using \mypddl, \emph{-sub} integrates new, gen and loc, aiming at a
...
Package Control or by placing the files of \mypddl \footnote{The files can be downloaded from \url{https://github.com/Pold87/ba-thesis/}.} in the packages
...
(\texttt{View->Syntax->\textbackslash{}pddl}).
...
found in table \ref{tab:criteria} \ref{tab:criteria}.
...
\begin{enumerate}
\item Syntax Highlighting Tasks
\label{sec-5-4-6-1}
\item Type Diagram Tasks
\label{sec-5-4-6-2}
...
can be seen in Figure xyz \ldots{}:For example, a the \pddl domain
