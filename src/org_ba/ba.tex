...
\section{Abstract}
\label{sec-1}
\subsection{{\bfseries\sffamily TODO} Insert somewhere}
\label{sec-1-1}
...
\subsection{{\bfseries\sffamily TODO} Notes}
\label{sec-2-1}
...
problem file. A planner and use the generated solution file (\emph{plan}) (more
extensive domains can be found on my website \footnote{\url{http://pold87.github.io/pddl-domains}}.
...
in \footnote{\url{http://www.plg.inf.uc3m.es/ipc2011-deterministic/attachments/OtherContributions/kovacs-pddl-3.1-2011.pdf}}.
This thesis makes use of PDDL 3.1 if not otherwise stated. 

Every planning task in PDDL is composed of two separate files:

\begin{itemize}
\item Domain file: description of general types, predicates, functions and actions
\item Problem file: description of a concrete problem environment.
\end{itemize}
...
\subsubsection{{\bfseries\sffamily TODO} Include simple domain -> \LaTeX{}}
\label{sec-3-3-1}
\subsubsection{{\bfseries\sffamily TODO} Include simple problem -> \LaTeX{}}
\label{sec-3-3-2}
\subsubsection{{\bfseries\sffamily TODO} Include simple plan -> not yet in \LaTeX{}}
\label{sec-3-3-3}
...
\label{sec-4}
\subsection{Kitchen Domain}
\label{sec-4-1}
In this section, I will present a kitchen domain, whereby PDDL
...
An Euclidean distance function that uses the square root would be convenient for distance modeling and measurement. However, PDDL (v3.1) supports only four arithmetic operators (+, -, /, *)\cite These operators can be used in preconditions, effects (normal/continuous/conditional) and durations. \cite{parkinson2012increasing} describe a workaround for this drawback. By declaring an action `calculate-sqrt', they bypass the lack of this function and rather write their own action that makes use of the Babylonian root method.
...
\subsection{...}
\section{Human Planner Interaction}
\section{World State - Plan Stepper}
\% EMail + SMS connection

\% Very interesting:
\% \url{http://www.tzi.de/~edelkamp/modplan/}



\% Reuse existing plans (VAL!). If a plan is not reusable, find problems  (e.g. step 13 and 17) and try to find solutions for that

\% Summary: List all limits and possibilities and write 5 sentences for
each  statement (my own idea!)
\section{Software Engineering Tools for AI Planning}
\label{sec-5}
\subsection{{\bfseries\sffamily TODO} Ideas}
\label{sec-5-1}
\begin{itemize}
...
\begin{description}
\item[{Section 1}] The automation of the PDDL task design process. File
input and output and dynamic generation (design level)
\item[{Section 2}] An interactive PDDL environment: speech synthesis and
recognition.
\item[{Section 3}] Mathematical limitations (design level)
\end{description}
\subsection{Implementation}
\label{sec-5-3}
\subsubsection{Integrated Design}
\label{sec-5-3-1}
...
\#+TODO TODO
\subsubsection{Syntax Highlighting and Code Snippets}
\label{sec-5-3-2}
\begin{enumerate}
...
AAAPackageDev\textcite{aaapackagedev}. So, the definitions can be
...
should be highlighted. By removing (or commenting) include 
...
\item PDDL Mode
\label{sec-5-3-2-4-2}
Announced 2005 in a mailing list entry, PDDL mode supports PDDL 2.1
\item itSimple
\label{sec-5-3-2-4-3}
\end{enumerate}
\end{enumerate}
